...
import siteConfiguration from './make/site.json'
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration, base: string): Plugin {
  ...
  // Resolve a root-relative public path (e.g. "/favicon.png") against the
  // configured base, so it still resolves when the site is served from a
  // subpath (like a GitHub Pages project page).
  ...
  const favicon = resolvePublicPath(config.icons?.icon ?? '')
  const socialImage = resolvePublicPath(config.openGraph?.image ?? '')
